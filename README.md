...
- JessieCode lexical analysis, the first statement/expression AST/parser
  ...
  statement lists, blocks, `if`/`else`, assignment, literals, variables,
  mutable arrays and objects, calls, properties, indexes, conditionals,
...
JessieCode loops, return/use/delete statements, function/map syntax, nested
and persistent mutable scopes, the complete built-in and element `methodMap`
surface, and the JSXGraph construction parser are not yet translated, and the
element renderer is incomplete.

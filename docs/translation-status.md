...
| `src/parser/jessiecode.js` | generated jison-lex token rules; generic AST nodes; empty programs, expression statement lists, blocks, `if`/`else`, `while`/`do`/`for`, return, and deprecated delete statements covering right-associative assignment, literals, variables, mutable arrays/objects, calls, properties, indexes, conditionals, and unary/binary precedence; interpreter support for that subset, JavaScript truthiness and primitive/array coercion, lazy branch evaluation, ordered loop evaluation, Board element deletion, object-property construction, array holes/length/properties, short-circuit evaluation, core math calls, board element lookup through a bounded adapter, stable name-to-ID replacement, ID-to-current-name restoration, and static dependency discovery; bounded lexing, parsing, evaluation, loop execution, collection growth, name replacement/restoration, and dependency traversal; the deprecated multi-Board `use` statement, function/map syntax, nested and persistent mutable scopes, element mutation through `setProp`, the complete built-in set, and the complete element `methodMap` remain pending | `parser/JessieCodeLexer.kt`, `parser/JessieCodeAst.kt`, `parser/JessieCodeExpressionParser.kt`, `parser/JessieCodeRuntime.kt`, `parser/JessieCodeEvaluator.kt`, `parser/JessieCodeNameReplacer.kt`, `parser/JessieCodeDependencyCollector.kt` | Partially translated |

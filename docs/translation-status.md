...
| `src/base/coordselement.js` | GeometryElement integration; coordinate storage; glider/constraint base state; `X`, `Y`, `Z`, `Coords`, evaluator, distance accessors, and the translated read-only JessieCode `methodMap` subset; and the free untransformed `updateCoords` / `setPositionDirectly` lifecycle | `base/CoordsElement.kt`, `parser/CoreGeometryElementRuntime.kt` | Translated dependency subset |

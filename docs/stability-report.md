...
3. **67 development parity scenarios.** These remain focused regression
...
The focused two-dimensional Ticks fixture verifies fixed Line/Curve ticks,
source-index labels, faced and one-sided infinite paths, and hidden polar
ticks inherited from an invisible parent. It scored `0.987918` on Desktop and
`0.983795` on Compact. Both contact sheets passed manual review; official
...
The focused two-dimensional Grid fixture verifies linked major/minor Curves,
explicit hidden Axis parents, and theme-six circle/point rendering. It scored
`0.988817` on Desktop and `0.980518` on Compact. Both contact sheets passed
manual review; official fixtures and JVM tests additionally cover themes
`0..6`, every documented face, automatic and user/`px`/`%`/`fr` spacing,
square forcing, boundaries, zero-axis controls, nested identity, lifecycle,
persistent sessions, pre-allocation limits, and atomic rollback. This
evidence remains outside the 30-case Stable production corpus.
The focused two-dimensional Image fixture verifies an untransformed
four-quadrant base64 PNG and a rotated translucent copy from the same source.
It scored `0.985178` on Desktop and `0.985329` on Compact. Both contact sheets
passed manual review for lower-left anchoring, quadrant orientation, affine
rotation, opacity, clipping, overlap, and blank output. Official fixtures and
common tests additionally cover dynamic source/coordinate/size updates,
`setSize`, negative dimensions, method-map access, source and decoded-byte
limits, resolver failures, and screen-basis geometry. This evidence remains
...
contains 30 production and 67 focused cases, but this does not change the
...
Polygon proxies, including open 3D vertex storage, closed proxy geometry, and
nested vertex/border styles. Static captures scored `0.988028` on Desktop and
`0.982365` on Compact. Both contact sheets passed manual review for projected
geometry, closure, fill, borders, ownership, overlap, clipping, and blank
output. This evidence remains outside the Stable production corpus.
The focused `curve3d_projection` fixture verifies component functions, a
vector-valued function, a discrete point matrix, and a transformed Curve3D
through ordinary Curve proxies. Static captures scored `0.988916` on Desktop
...
The focused `vectorfield3d_projection` fixture verifies component and
array-returning VectorField3D definitions, inclusive three-axis meshes,
zero-vector suppression, scale, `NaN` path breaks, and viewport-sized
arrowheads before parallel View3D projection. Static captures scored
`0.988321` on Desktop and `0.984174` on Compact. Both contact sheets passed
manual review for vector placement, arrow direction and size, color,
